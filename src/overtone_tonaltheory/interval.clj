...
;A vector of vectors of enharmonically equivalent intervals (represented as keywords) in order from smallest to largest, ranging
;from unison to major 10th. The array index of an enharmonic group is equivalent to the
...
(defn interval-keywords
  "Returns a vector containing subvectors of the keywords that represent the interval between the two notes.
  The two notes must be in midi note format.
  The individual subvectors consist of the enharmonically-equivalent keys for the interval (for example, a
  subvector containing major 3rd and diminished 4th), while the vector contains those subvectors.
